...
pub fn run(KEY:&'static str, METHOD:&'static EncoderMethods, SERVER_ADDR:&'static str, 
    ...
    handle_tun_data(iface, KEY, METHOD, SERVER_ADDR, PORT_START, PORT_END, BUFFER_SIZE, first_packet);
}


fn handle_tun_data(tun: linux::Device, KEY:&'static str, METHOD:&'static EncoderMethods, 
        ...
        loop {
            index += match stream_read.read(&mut buf[index..]) {
                ...
                _ => {
                    ...
                    let server_new = match client::tun_get_stream(KEY, METHOD, SERVER_ADDR, PORT_START, PORT_END, &[], 0){
                        Some((stream, encoder)) => Server {stream, encoder},
                        None => continue
                    };
